# this has to be done in python2 for some reason..
import pickle
import os
import csv
import random
...
def gen_test(reduced_dict):
  test = []
  for picture_name in reduced_dict:
    inputs = list(reduced_dict[picture_name])
    outputs = [0,0]
    missing_dims = [0] * len(inputs)
    test.append([2, 14, picture_name] + inputs + outputs + missing_dims)
  return test
...
def load_PCA_data():
  ...
  for reduced_file in os.listdir(data_dir):
    if ".json" in reduced_file:
      with open(str(os.path.join(data_dir, reduced_file)), "r") as file:
        save_traning_data(json.load(file), training_cats, test, reduced_file)
